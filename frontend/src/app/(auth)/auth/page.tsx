...
import { toastSuccess, toastError, toastInfo } from "@/utility/toastStyle";
...
function AuthContent() {
  ...
  const redirectPath = searchParams.get("from") || "/send";
  ...
  useEffect(() => {
    if (error) {
      if (activeTab === "login") {
        ...
        toastError("Signup failed", { description: error });
      }
    } else {
      setErrors({});
    }
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const success = await login(loginForm, redirectPath);
      if (success) {
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    ...
    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
      toastInfo("Please fill all required fields correctly");
      return;
    }

    setErrors({});

    const success = await register(signupForm, redirectPath);
    if (success) {
    ...
    if (error) {
      // Error handling is done in the useEffect above
      // so we don't need to do anything here
      // setErrors({ signupAuthError: error });
      // toastError("Signup failed", { description: error });
  };
}
            ...
            >
              ...
              >
                ...
                <p className="text-lg mb-8 text-blue-50/90">
                  Share files securely with password protection and expiration
                  dates. Get started with Aerofy today.
                </p>
              </motion.div>
                  ...
                  {activeTab === "login" ? (
                    ...
                    >
                      <form onSubmit={handleLoginSubmit} className="space-y-4">
                        {errors.authError && (
                          <div className="p-3 mb-3 text-sm rounded-md bg-destructive/10 text-destructive border border-destructive/20">
                            {errors.authError}
                          </div>
                        )}
